...
export default function EditResep() {
  ...
  const updateResep = useMutation(api.resep.update);
  ...
  useEffect(() => {
    if (resep) {
      setForm({
        name: resep.name,
        durasi: resep.durasi?.toString() || "",
        tingkatKesulitan: resep.tingkatKesulitan || "",
        foto: resep.foto || "",
      porsi: resep.porsi?.toString() || "",
        bahan: resep.bahan.length > 0
          ? resep.bahan.map((b) => ({ nama: b.nama || "", jumlah: b.jumlah ?? 0, satuan: b.satuan || "" }))
          : [{ nama: "", jumlah: 0, satuan: "" }],
        langkah: resep.langkah.length > 0
          ? resep.langkah.map((l) => l || "")
          : [""],
        tips: resep.tips || "",
        kategori: resep.kategori || [],
      });
      setYoutubeUrl(resep.youtubeUrl || "");
    }
  }, [resep]);
  ...
  return (
    <div className="mx-auto max-w-xl px-4 sm:px-6 pt-6">
      <div className="mb-6 inline-flex items-center gap-2">
        ...
        <h1 className="text-xl font-bold tracking-tight">Edit Resep</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="pb-10">
        ...
        <div className="mb-6">
          ...
          <input
            placeholder="Tambah kategori sendiri (pisah dengan koma, tekan Enter)"
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
